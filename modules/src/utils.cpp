...
void hwc_to_chw(cv::InputArray src, cv::OutputArray dst) {
    const i32 src_h = src.rows();
    const i32 src_w = src.cols();
    const i32 src_c = src.channels();

    cv::Mat hw_c = src.getMat().reshape(1, src_h * src_w);

    const std::array<i32,3> dims = {src_c, src_h, src_w};                         
...
void chw_to_hwc(cv::InputArray src, cv::OutputArray dst) {                      
    ...
    const i32 src_c = src_size[0];                                                
    const i32 src_h = src_size[1];                                                
    const i32 src_w = src_size[2];                                                
