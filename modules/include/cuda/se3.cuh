...
public:
    ...
    __host__ __device__ SE3(const Type qw, const Type qx, const Type qy, const Type qz, 
    ...
    __host__ __device__ SE3(const float4& qaut, const float3& trans) {
    ...
    __host__ __device__ SE3(const Type r[9], const Type t[3]) {
    ...
    __host__ __device__ SE3<Type> inv() const {
        SE3<Type> result;
    ...
    __host__ __device__ float3 rotate(const float3& p) const {
    ...
    __host__ __device__ float3 translate(const float3& p) const {
        return make_float3(p.x + se3[0][3],
                           p.y + se3[1][3],
                           p.z + se3[2][3]);
    }

    __host__ __device__ float3 operator*(const float3& p) {
    ...
    __host__ __device__ inline Type& operator()(const i32 row, const i32 col) {
        if (row >= 3 || row < 0 || col >= 4 || col < 0) {
            return se3[0][0];
        }
    ...
    __host__ __device__ inline Type* data() const { 
        return se3; 
    }
private:
    __host__ __device__ inline void init(const Type wx, const Type wy, const Type wz, const Type xx,
...
template<typename Type>
__host__ __device__ SE3<Type> operator*(const SE3<Type> &lhs, const SE3<Type> &rhs) {
    SE3<Type> result;
